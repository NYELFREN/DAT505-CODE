...
```
This code defines three Sprite, gives them different positions and the same initial color, and then gives the mouse a way to move, so that when the mouse is placed on the object, the color of the object will change with the change of the mouse position.
```javascript
  // add sprites
