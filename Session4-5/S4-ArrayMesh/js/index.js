...
function init() {
  ...
  for (var x = -10; x <= 5; x += 5) { // Start from -45 and sequentially add one every 5 pixels
  ...
  for (var z = -10; z <= 5; z += 5) {
      ...
      boxMaterial = new THREE.MeshLambertMaterial({color: 0x6C5B7B});
    } else if ( x <= 0 && y <= 0 && z >=0){
      boxMaterial = new THREE.MeshLambertMaterial({color: 0x355C7D});
    } else if ( x <= 0 && y >= 0 && z <=0){
      boxMaterial = new THREE.MeshLambertMaterial({color: 0xF02A694});
    } else if ( x >= 0 && y <= 0 && z <=0){
      boxMaterial = new THREE.MeshLambertMaterial({color: 0xEFB704});
    } else {
      boxMaterial = new THREE.MeshLambertMaterial({color: 0xF2355B});
    }
